...
segmSpecialFilter cond sizes arr =
    ...
    ------------------------------------------------
    let cs = map cond arr
        -- list of where the predicates true
        tfs = map (\f-> if f then 1 else 0) cs
        isT = segmScanInc (+) 0 sizes tfs

        --Creates a list where each segment ends f.ex. [2,0,1] flagarray
        -- would become [2,2,3].
        ix = (map (subtract 1) . scanInc (+) 0) sizes
        is = map (\i -> isT !! i) ix
        
        -- Follows parFilters principle, where isF indicates the false index
        -- in the result array
        ffs = map (\f -> if f then 0 else 1) cs
        isF = zipWith (+) (segmScanInc (+) 0 sizes ffs) is
        --Indexes within its own segment
        inds = map (\(c, iT, iF) -> 
                if c then iT-1 else iF-1) (zip3 cs isT isF)
        -- Calculating offsets
        rF = zipWith (-) (segmScanInc (+) 0 sizes sizes) (scanInc (+) 0 sizes) 
        indexes = zipWith (-) inds rF

        flag = map (\s -> if (s>0) then is!!s else 0) sizes
        --flag2 = 
        in ([], permute indexes arr)
