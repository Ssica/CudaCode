        ...
        lssop (lssx, lisx, lcsx, tlx, firstx, lastx, okx)
          ...
          (newlss, newlis, newlcs, tlx+tly, firstx, lasty, newok)
                where
                  ...
                  newlss  = if connect then (lcsx + lisy) `max` lssx `max` lssy else lssx `max` lssy
                  newlis  = if connect && okx then lssx +lisy else lisx
                  newlcs  = if connect && oky then lcsx + lcsy else lcsy
